  ...
  factory: (require) => {
    ...
    function IdleSchedulerSection() {
      ...
      const submit = (mode, runAt) => {
        ...
        fetch("/api/idle-scheduler/tasks", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) })
          .then((r) => r.json())
          .then((d) => { setMsg(d.ok ? (mode === "scheduled" ? "已加入定时队列" : "已加入闲时队列") : ("失败：" + (d.error || ""))); setPrompt(""); setOpenSched(false); setSchedAt(""); refresh(); })
    ...
    function IdleComposerButton(props, draftText) {
      ...
      const submit = (mode) => {
        ...
        fetch("/api/idle-scheduler/tasks", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) })
          .then((r) => r.json())
          .then((d) => { setMsg(d.ok ? (mode === "scheduled" ? "已加入定时队列" : "已加入闲时队列") : ("失败：" + (d.error || ""))); if (d.ok) clearComposerDraft(props); })
          .catch((e) => setMsg("请求失败：" + e))
          .finally(() => setBusy(false));
      };
